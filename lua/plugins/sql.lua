  ...
  },
  {
    ...
    init = function()
      ...
      vim.g.db_ui_table_helpers = {
        postgresql = {
          Insert = [[SELECT 
          'INSERT INTO {table} ('||STRING_AGG(column_name, ', ')||') VALUES'||
          '('||STRING_AGG('{'||column_name||'::'||data_type||CASE WHEN is_nullable = 'YES' THEN '?' ELSE '' END||'}', ', ')||')' 
          FROM information_schema.columns WHERE table_schema = '{schema}' AND table_name = '{table}' GROUP BY table_name;
          ]],
        },
      }
